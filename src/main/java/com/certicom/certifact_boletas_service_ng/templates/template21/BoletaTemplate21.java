package com.certicom.certifact_boletas_service_ng.templates.template21;
...
public class BoletaTemplate21 {
    ...
    public String buildBoleta21(PaymentVoucherDto invoice) throws TemplateException {
        String xml = "";
        DOMSource source;
        StringWriter writer;
        ...
        try {
            ...
            */
            ...
            if (invoice.getLeyendas() != null && !invoice.getLeyendas().isEmpty()) {
                for (LeyendaDto note : invoice.getLeyendas()) {
            ...
            if(invoice.getCamposAdicionales()!=null && !invoice.getCamposAdicionales().isEmpty()){
                for (AditionalFieldPaymentVoucherDto adicional : invoice.getCamposAdicionales()){
                    appendChild(doc, invoiceRootElement, "cbc:Note","<![CDATA["+
                            adicional.getNombreCampo()+":"+adicional.getValorCampo()+"]]>");
                }
            ...
            if (invoice.getGuiasRelacionadas() != null && !invoice.getGuiasRelacionadas().isEmpty()) {
                for (GuiaPaymentVoucherDto guiaRelacionada : invoice.getGuiasRelacionadas()) {
            ...
            if (StringUtils.isNotBlank(invoice.getCodigoTipoOperacion())
                    && invoice.getCodigoTipoOperacion()
                    .equals(ConstantesSunat.CODIGO_TIPO_OPERACION_VENTA_INTERNA_ANTICIPOS)
                    && invoice.getAnticipos() != null && !invoice.getAnticipos().isEmpty()) {

                for (AnticipoPaymentDto anticipo : invoice.getAnticipos()) {
                    ...
                    appendChild(doc, additionalDocument, "cbc:DocumentTypeCode", anticipo.getTipoDocAnticipo());
            ...
            if (invoice.getDocumentosRelacionados() != null && !invoice.getDocumentosRelacionados().isEmpty()) {
                for (DocumentoRelacionadoDto documentoRelacionado : invoice.getDocumentosRelacionados()) {
            ...
            if (StringUtils.isNotBlank(invoice.getCodigoBienDetraccion())) {
                ...
                appendChild(doc, paymentTermsElement, "cbc:Amount", UtilFormat.format(invoice.getMontoDetraccion()))
            ...
            if (StringUtils.isNotBlank(invoice.getCodigoTipoOperacion())
                ...
                appendChildAllowanceAnti(doc,invoiceRootElement,
                        montoAnticiposTotalValorVenta,invoice.getCodigoMoneda(),"04",invoice.getTotalValorVentaGravada());
            }
            // [Anticipos]
            ...
            for (DetailsPaymentVoucherDto item : invoice.getItems()) {
                ...
                addElementsChildSubTaxTotal(doc, taxTotalElement, item.getMontoBaseGratuito(),
                        ...
                        invoice.getCodigoMoneda(), item.getPorcentajeTributoVentaGratuita(), false, attributes);
    ...
    private void appendChildDeliveryDetraccion(Document doc, Element cacInvoiceLine, DetailsPaymentVoucherDto item, Map<String, String> attributes) {
